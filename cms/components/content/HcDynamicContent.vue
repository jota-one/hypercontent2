...
const resolveSlot = (slot?: Slot<any>) => {
  if (slot) {
    const resolveProps = (componentProps: Record<string, any>) =>
      Object.entries(componentProps).reduce(
        (acc: Record<string, any>, [key, value]) => {
          if (typeof value === 'object') {
            ...
            if (propValue === entityRef) {
              ...
              propValue.indexOf(entityRef) > -1
            ) {
              acc[key] = propValue
                .split(entityPropRegExp.value)
...
const fetchEntity = async (entity: string, field: string) => {
  ...
  const apiEndpointPath = resolveEndpointDefPlaceholders(
    endpoint.detail.path,
    entityObj
  )
  const apiEndpointQueryParams = resolveEndpointDefPlaceholders(
    endpoint.detail.queryParams,
    { lang }
  )
