...
export const useHcLabels = () => {
  const labels = useState<Record<string, Record<string, string>>>(
    'hcLabels',
    () => ({})
  )
  ...
  const langCode = computed(
    () => currentLangCode.value || defaultLang.value?.code || 'en'
  )

  const _loadLabels = async () => {
    const hcApiPath = `${hypercontent.content.api.base}${hypercontent.content.api.labels.replace(
      '__langCode__',
      langCode.value
    )}`
  ...
  const l = computed(() => (key: string, values?: Record<string, string>) => {
    return (
      computeLabel(' ', labels.value, langCode.value)(
        key,
        values || {},
        null,
        false
